...
export function tuneingPath(source: Vector2D, target: Vector2D, portals: [Vector2D, Vector2D][],
	...
	const debug = (vect:Vector2D,color:string)=>{
		if(debugCtx && worldToScreen){
			const vec = worldToScreen(vect)
			debugCtx.beginPath()
			debugCtx.arc(vec.x,vec.y,4,0,Math.PI*2);
