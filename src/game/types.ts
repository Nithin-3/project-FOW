...
function vectorEquals(a: Vector2D, b: Vector2D): boolean {
	return a.x === b.x && a.y === b.y;
}

function areAdjacent(a: Vector2D, b: Vector2D, polygon: Polygon): boolean {
...
export class GameObject {
	...
	color: Color;
	wall: boolean;
	private _boundingBox: { p1: Vector2D; p2: Vector2D; } = { p1: { x: Infinity, y: Infinity }, p2: { x: -Infinity, y: -Infinity } };
	constructor(zIndex: number, points: Polygon, color: Color, doors?: Vector2D[]) {
		...
		for (const p of points) {
			if (p.x < this._boundingBox.p1.x) this._boundingBox.p1.x = p.x;
			if (p.y < this._boundingBox.p1.y) this._boundingBox.p1.y = p.y;
			if (p.x > this._boundingBox.p2.x) this._boundingBox.p2.x = p.x;
			if (p.y > this._boundingBox.p2.y) this._boundingBox.p2.y = p.y;
		}
	}

	public get boundingBox(): { p1: Vector2D; p2: Vector2D; } {
